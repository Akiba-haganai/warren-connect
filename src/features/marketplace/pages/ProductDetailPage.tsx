...
import {
  ArrowLeft, MessageCircle, Share2, Loader2, ShieldCheck
} from "lucide-react";
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    productService.getProductWithSeller(id).then(setProduct).finally(() => setLoading(false));
  }, [id]);
  ...
  const handleContactSeller = async () => {
    ...
    try {
      ...
      // ✅ Fixed: added product.id and product.title as separate arguments
      ...
      <div className="px-4 pt-4 pb-8 space-y-4">
        ...
        {product.seller && (
          <div className="card p-4 flex items-center gap-4">
            {product.seller.avatar_url ? <img src={product.seller.avatar_url} alt="" className="w-10 h-10 rounded-full" /> :
             <div className="w-10 h-10 rounded-full bg-primary flex items-center justify-center text-white font-bold">{(product.seller.full_name?.[0] ?? "?")}</div>}
            <div className="flex-1">
              <p className="font-semibold text-sm flex items-center gap-1" style={{ color: "var(--color-text)" }}>
            ...
            {!isOwner && (
              <button onClick={handleContactSeller} disabled={contacting} className="btn-primary w-auto px-4 py-2 text-sm"
