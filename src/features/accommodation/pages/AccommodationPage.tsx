...
export default function AccommodationPage() {
  ...
  const locations = useMemo(() => [...new Set(allListings.map((l) => l.location))].sort(), [allListings]);
  ...
  useEffect(() => {
    if (allListings.length === 0) return;
    Promise.all(allListings.map(async (acc) => {
      const ams = await accommodationService.getAmenities(acc.id);
      return { id: acc.id, amenities: ams };
    })).then((results) => {
      const map: Record<string, string[]> = {};
      results.forEach((r) => { map[r.id] = r.amenities; });
      setAllAmenities(map);
    });
  }, [allListings]);
    ...
    <div style={{ background: "var(--color-bg)", minHeight: "100%" }}>
      ...
      <div className="px-4 pt-4 pb-8 flex flex-col gap-4">
        ...
        <AccommodationFilters
          ...
          locations={locations}
          roommateFilter={roommateFilter} onRoommateChange={setRoommateFilter}
          genderFilter={genderFilter} onGenderChange={setGenderFilter}
          priceMin={priceMin} onPriceMinChange={setPriceMin}
        ...
        {status === "pending" ? (
          ...
          <div className="rounded-2xl py-16 text-center" style={{ background:"var(--color-surface)", border:"1px dashed var(--color-border)" }}>
            ...
            <h3 className="text-lg font-bold mb-1" style={{ color:"var(--color-text)" }}>
              {search || locationFilter || roommateFilter || genderFilter || priceMin || priceMax || selectedAmenities.length>0 ? "No listings match your criteria" : "No listings yet"}
            </h3>
            <p className="text-sm mb-4" style={{ color:"var(--color-text-secondary)" }}>
              {search || locationFilter || roommateFilter || genderFilter || priceMin || priceMax || selectedAmenities.length>0 ? "Try adjusting your filters." : "Add the first accommodation!"}
            </p>
            {!search && !locationFilter && !roommateFilter && !genderFilter && !priceMin && !priceMax && selectedAmenities.length===0 && (
          ...
          <>
            {filtered.map((listing) => {
              ...
              return (
                <AccommodationCard key={listing.id} listing={listing} landlord={landlord} />
              );
            })}
      ...
      {showComposer && <AccommodationComposer onClose={() => setShowComposer(false)} onCreated={handleCreated} />}
    </div>
  );
}