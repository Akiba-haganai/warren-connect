...
serve(async (req) => {
  ...
  try {
    ...
    // Option 3: Strictly use Supabase's native auth mailer (3/hour limit)
    const { error: resetErr } = await supabaseAdmin.auth.resetPasswordForEmail(reqRow.email, { 
      redirectTo: `${siteUrl}/reset-password` 
    });

    if (resetErr) {
      return new Response(
        JSON.stringify({ error: `Supabase Auth Mailer Error: ${resetErr.message}` }),
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Password reset email approved & dispatched to ${reqRow.email} via native mailer.`,
      }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err: any) {
